          ...
          {showAdvanced ? (
            <div>
              <label class={style.optionTextFirst}>
                ...
                >
                  <option value={MozJpegColorSpace.GRAYSCALE}>{t('mozjpeg.grayscale')}</option>
                  <option value={MozJpegColorSpace.RGB}>{t('mozjpeg.rgb')}</option>
                  <option value={MozJpegColorSpace.YCbCr}>{t('mozjpeg.ycbcr')}</option>
                </Select>
              </label>
              <Expander>
                      ...
                      {options.auto_subsample ? null : (
                        <div class={style.optionOneCell}>
                          <Range
                            ...
                            onInput={this.onChange}
                          >
                            {t('mozjpeg.subsampleBy')}
                          </Range>
                        </div>
                      )}
                    </Expander>
                    <label class={style.optionToggle}>
                      {t('mozjpeg.separateChromaQuality')}
                      <Checkbox
              ...
              <label class={style.optionToggle}>
                {t('mozjpeg.pointlessSpecCompliance')}
                <Checkbox
                  name="baseline"
              ...
              <Expander>
                {options.baseline ? null : (
                  <label class={style.optionToggle}>
                    {t('encoder.progressiveRendering')}
                    <Checkbox
                      name="progressive"
              ...
              <label class={style.optionTextFirst}>
                {t('mozjpeg.quantization')}
                <Select
                  name="quant_table"
                  value={options.quant_table}
                  onChange={this.onChange}
                >
                  <option value="0">{t('mozjpeg.quant.annexK')}</option>
                  <option value="1">{t('mozjpeg.quant.flat')}</option>
                  <option value="2">{t('mozjpeg.quant.mssim')}</option>
                  <option value="3">{t('mozjpeg.quant.imagemagick')}</option>
                  <option value="4">{t('mozjpeg.quant.psnrHvsM')}</option>
                  <option value="5">{t('mozjpeg.quant.klein')}</option>
                  <option value="6">{t('mozjpeg.quant.watson')}</option>
                  <option value="7">{t('mozjpeg.quant.ahumada')}</option>
                  <option value="8">{t('mozjpeg.quant.peterson')}</option>
                </Select>
              </label>
              <label class={style.optionToggle}>
                {t('mozjpeg.trellisMultipass')}
                <Checkbox
              ...
              <Expander>
                {options.trellis_multipass ? (
                  <label class={style.optionToggle}>
                    {t('mozjpeg.optimizeZeroBlockRuns')}
                    <Checkbox
              ...
              <label class={style.optionToggle}>
                {t('mozjpeg.optimizeAfterTrellis')}
                <Checkbox
