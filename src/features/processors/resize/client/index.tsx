    ...
    return (
      ...
      >
        <label class={style.optionTextFirst}>
          ...
          >
            {isVector && (
              <option value="vector">{t('resize.method.vector')}</option>
            )}
            <option value="lanczos3">{t('resize.method.lanczos3')}</option>
            <option value="mitchell">{t('resize.method.mitchell')}</option>
            <option value="catrom">{t('resize.method.catmullRom')}</option>
            <option value="triangle">{t('resize.method.triangle')}</option>
            <option value="hqx">{t('resize.method.hqx')}</option>
            <option value="browser-pixelated">
              {t('resize.method.browserPixelated')}
            </option>
            <option value="browser-low">{t('resize.method.browserLow')}</option>
            <option value="browser-medium">
              {t('resize.method.browserMedium')}
            </option>
            <option value="browser-high">
              {t('resize.method.browserHigh')}
            </option>
          </Select>
        </label>
        <label class={style.optionTextFirst}>
          ...
          <Select value={this.getPreset()} onChange={this.onPresetChange}>
            ...
            <option value="custom">{t('resize.preset.custom')}</option>
          </Select>
        </label>
        <label class={style.optionTextFirst}>
          {t('resize.width')}
          ...
          {maintainAspect ? null : (
            <label class={style.optionTextFirst}>
              ...
              <Select
                ...
                onChange={this.onChange}
              >
                <option value="stretch">{t('resize.fitMethod.stretch')}</option>
                <option value="contain">{t('resize.fitMethod.contain')}</option>
              </Select>
            </label>
          )}
        </Expander>
