...
export default class App extends Component<Props, State> {
  ...
  constructor() {
    ...
    compressPromise
      ...
      .catch(() => {
        this.showSnack(t('app.loadFailed'));
      });
  ...
  /** One image opens the editor; several open batch mode. */
  private openFiles = (files: File[]) => {
    if (files.length === 1) {
      this.openEditor();
      ...
      <div class={style.app}>
        <file-drop onfiledrop={this.onFileDrop} class={style.drop}>
          {showSpinner ? (
            ...
            Batch && (
              <Batch files={files!} showSnack={this.showSnack} onBack={back} />
            )
          ) : isEditorOpen ? (
            Compress && (
