...
class WorkerBridge {
  ...
  /**
   * @param threads Cap on the cores one encode may use. Threaded wasm codecs
   * read this from the worker URL via `worker-shared/thread-limit`.
   */
  constructor(protected threads?: number) {}
  ...
  protected _startWorker() {
    const url = this.threads
      ? `${workerURL}?threads=${encodeURIComponent(this.threads)}`
      : workerURL;
