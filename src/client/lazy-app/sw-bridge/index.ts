...
export async function offliner(showSnack: SnackBarElement['showSnackbar']) {
  // The desktop shell serves the app over a custom protocol with no service
  // worker; skip all offline/update handling there.
  ...
  navigator.serviceWorker.addEventListener('controllerchange', async () => {
    ...
    if (!hasController) {
      showSnack(t('sw.offlineReady'), { timeout: 5000 });
      return;
    }
  ...
  const result = await showSnack(t('sw.updateAvailable'), {
