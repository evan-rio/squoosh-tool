...
export default class Compress extends Component<Props, State> {
  ...
  private onCopyToOtherClick = async (index: 0 | 1) => {
    ...
    const result = await this.props.showSnack(t('editor.snack.settingsCopied'), {
      timeout: 5000,
  ...
  private onSaveSideSettingsClick = async (index: 0 | 1) => {
    if (index === 0) {
      ...
      await this.props.showSnack(t('editor.snack.leftSaved'), {
        timeout: 1500,
        actions: ['dismiss'],
      });
      return;
    ...
    if (index === 1) {
      ...
      await this.props.showSnack(t('editor.snack.rightSaved'), {
        timeout: 1500,
        actions: ['dismiss'],
      });
      return;
  ...
  private onImportSideSettingsClick = async (index: 0 | 1) => {
    ...
    if (index === 0 && leftSideSettingsString) {
      ...
      const result = await this.props.showSnack(t('editor.snack.leftImported'), {
        timeout: 3000,
        actions: ['undo', 'dismiss'],
      });
      if (result === 'undo') {
  ...
  private async updateImage() {
    ...
    if (needsDecoding) {
      try {
        ...
        this.props.showSnack(t('editor.error.decode', { error: String(err) }));
        throw err;
      }
    } else {
    ...
    if (needsPreprocessing) {
      try {
        ...
        this.props.showSnack(t('editor.error.preprocess', { error: String(err) }));
        throw err;
      }
    } else {
    ...
    sideWorksNeeded.forEach(async (sideWorkNeeded, sideIndex) => {
      try {
        ...
        this.props.showSnack(t('editor.error.process', { error: String(err) }));
        throw err;
      }
    });
  }
      ...
      <Results
        ...
        typeLabel={
          side.latestSettings.encoderState
            ...
            : `${side.file ? `${side.file.name}` : t('editor.originalImage')}`
        }
      />
    ));
