...
const demos = [
  {
    description: 'intro.demo.largePhoto',
    size: '2.8MB',
    filename: 'photo.jpg',
    url: largePhoto,
  ...
  {
    description: 'intro.demo.artwork',
    size: '2.9MB',
    filename: 'art.jpg',
    url: artwork,
    iconUrl: artworkIcon,
  ...
  {
    description: 'intro.demo.deviceScreen',
    size: '1.6MB',
    filename: 'pixel3.png',
    url: deviceScreen,
  ...
  {
    description: 'intro.demo.svgIcon',
    size: '13KB',
...
export default class Intro extends Component<Props, State> {
  ...
  /**
   * The desktop shell would otherwise load external pages inside its own
   * window and lose the app UI, so hand them to the system browser.
   */
  private onExternalLink = (event: Event) => {
  ...
  private onDemoClick = async (index: number, event: Event) => {
    try {
      ...
      this.props.showSnack!(t('intro.error.fetchDemo'));
    }
  };
  ...
  private onPasteClick = async () => {
    ...
    try {
      ...
      this.props.showSnack!(t('intro.error.clipboardPermission'));
      return;
    }
    ...
    if (!blob) {
      this.props.showSnack!(t('intro.error.noImageInClipboard'));
      return;
    }
        ...
        <div class={style.demosContainer}>
          ...
          <div class={style.contentPadding}>
            <p class={style.demoTitle}>
              {t('intro.tryOnePrefix')}
              <strong>{t('intro.tryOneStrong')}</strong>
              {t('intro.tryOneSuffix')}
            </p>
            ...
            <div class={style.footerPadding}>
              <footer class={style.footerItems}>
                <a
                  ...
                  href="https://github.com/evan-rio/squoosh-tool"
                  onClick={this.onExternalLink}
                >
                  <img src={githubLogo} alt="" width="10" height="10" />
                  {t('intro.sourceOnGithub')}
                </a>
                <LocaleSelect />
              </footer>
            </div>
