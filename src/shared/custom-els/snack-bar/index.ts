...
import { t, MessageKey } from 'shared/i18n';

const actionMessageKeys: Record<string, MessageKey> = {
...
function createSnack(
  ...
  const result = new Promise<string>((resolve) => {
    ...
    for (const action of actions) {
      ...
      const messageKey = actionMessageKeys[action];
      button.textContent = messageKey ? t(messageKey) : action;
